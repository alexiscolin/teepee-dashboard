...
const createUserData = () => {
  ...
  for (let i = 0; i < userCount; i++) {
    ...
    const purchase = {
      ...
      estate_agent_cost: faker.datatype.float({
        min: 1000,
        max: propertyPrice,
      }),
      notary_cost: faker.datatype.float({ min: 1000, max: propertyPrice }),
      renovation_work_cost: faker.datatype.float({
        min: 1000,
        max: propertyPrice,
      }),
    };
    ...
    const loan = {
      loan_amount: faker.datatype.number({
        min: 1000,
        max:
          purchase.property_price +
          purchase.estate_agent_cost +
          purchase.notary_cost +
          purchase.renovation_work_cost,
      }),
